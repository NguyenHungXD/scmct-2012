<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeBehind="create_access_right_for_group.aspx.cs" Inherits="chiase.create_access_right_for_group" %>
            ...
            if (confirm("Xác nhận thay đổi!\n Chọn [OK] để tiếp tục, [Cancel] để Đóng.")) {
                if (obj.chk_all.length > 0) {
                    for (i = 0; i < obj.chk_all.length; i++) {
                            ...
                            var url = "create_access_right_for_group.aspx?groupid=" + groupid + "&moduleid=" + obj.chk_all[i].value + "&V=" + vView + "&C=" + vCreate + "&E=" + vEdit + "&D=" + vDel + "&L=" + vLock + "&vmode=" + vmode;
                            loadXMLUpdate(url);
                            checked = true;
                    }
                }
                        ...
                        var url = "create_access_right_for_group.aspx?groupid=" + groupid + "&moduleid=" + obj.chk_all.value + "&V=" + vView + "&C=" + vCreate + "&E=" + vEdit + "&D=" + vDel + "&L=" + vLock + "&vmode=" + vmode;
                        loadXMLUpdate(url);
                        checked = true;
                    }

                if (vmode == "create_new") {
                    var error = "Bạn chưa chọn quyền để xét cho nhóm thành viên!";
                    var result = "Cập nhật quyền cho nhóm thành viên thành công!";
                }
...
<fieldset>
    <asp:HiddenField ID="divgroupid" ClientIDMode="Static" runat="server" />
    <table border="0" cellpadding=2 cellspacing=1 width="100%"  style="border:0px solid #CCFFFF;">
        <tr style="color:White;font-weight:bold"><td colspan="2"><font size=3><p align="center">Xét quyền cho nhóm thành viên</p></font></td></tr>
        <tr>
        <td>
            Tên nhóm thành viên:
            ...
            <td>
                <font size=3><b><asp:Label ID="lbl_groupname" runat="server" Font-Size="Larger" Text=""></asp:Label></b></font>
        </td>
        </tr>
