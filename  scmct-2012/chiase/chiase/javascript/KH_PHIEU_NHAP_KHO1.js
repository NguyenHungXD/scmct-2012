...
function xoaontable(control, maphieu, bool) {
    if (bool || bool == null)
        $(control).XoaRow({
            valueConfirm: "Bạn có chắc là muốn xóa phiếu nhập " + maphieu + "?",
            valueErXoa: "Xóa không thành công vì phiếu nhập \"" + maphieu + "\" đang được sử dụng!",
            valueMesXoa: "Đã xóa thành công phiếu nhập \"" + maphieu + "\".",
            ajax: "ajax/KH_PHIEU_NHAP_KHO_ajax1.aspx?do=xoa"
        });
}
