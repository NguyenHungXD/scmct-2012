...
function xoaontable(control, maphieu, bool) {
    if (bool || bool == null)
        $(control).XoaRow({
            valueConfirm: "Bạn có chắc là muốn xóa phiếu chuyển " + maphieu + "?",
            valueErXoa: "Xóa không thành công vì phiếu chuyển \"" + maphieu + "\" đang được sử dụng!",
            valueMesXoa: "Đã xóa thành công phiếu chuyển \"" + maphieu + "\".",
            ajax: "ajax/KH_PHIEU_XUAT_CHUYEN_KHO_ajax1.aspx?do=xoa"
        });
}
