<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeBehind="update_album_imgs.aspx.cs" Inherits="chiase.update_album_imgs" %>
...
<script type="text/javascript">
    function close_page(id) {
        window.location = "update_album.aspx?album_id=" + id;
    }
    ...
    function save_info() {
        ...
        var obj = document.forms["chiase"];
        if (confirm("Xác nhận thay đổi!\nChọn [Ok] để lưu thông tin, [Cancel] để Đóng.")) {
            if (obj.txt_title.length > 0) {
                for (i = 0; i < obj.txt_title.length; i++) {
                    //Call ajax to update
                    if (obj.chk_del[i].checked == true) { //Mode = 1 Update info 2 Del 3 Set default image for album
                        var url = "update_album_imgs_save.aspx?img_id=" + obj.chk_del[i].value + "&mode=2";
                        loadXMLUpdate(url);
                    } else {
                        var url = "update_album_imgs_save.aspx?img_id=" + obj.chk_del[i].value + "&v_title=" + encodeURIComponent(obj.txt_title[i].value) + "&mode=1";
                        loadXMLUpdate(url);
                    }
                    if (obj.rd_mainimg[i].checked == true) {
                        var url = "update_album_imgs_save.aspx?img_id=" + obj.chk_del[i].value + "&album_id=" + obj.rd_mainimg[i].value + "&mode=3";
                        loadXMLUpdate(url);
                    }
                ...
                if (obj.chk_del.checked == true) { //Mode = 1 Update info 2 Del 3 Set default image for album
                    var url = "update_album_imgs_save.aspx?img_id=" + obj.chk_del.value + "&mode=2";
                    loadXMLUpdate(url);
                } else {
                    var url = "update_album_imgs_save.aspx?img_id=" + obj.chk_del.value + "&v_title=" + encodeURIComponent(obj.txt_title.value) + "&mode=1";
                    loadXMLUpdate(url);
                }
                if (obj.rd_mainimg.checked == true) {
                    var url = "update_album_imgs_save.aspx?img_id=" + obj.chk_del.value + "&album_id=" + obj.rd_mainimg.value + "&mode=3";
                    loadXMLUpdate(url);
                }
            }
...
Chi tiết: <b><%= album_description%></b>
<br>
<hr>
...
<asp:Repeater ID="showListimg" runat="server" >
        ...
        <td><br>
            <input style="cursor:pointer" name="rd_mainimg" id="<%#Eval("img_id") %>" type="radio" value="<%= albumid %>"/> <label for="<%#Eval("img_id") %>">Làm ảnh bìa album</label> 
        </td>
        <td><br>
        ...
        <tr><td colspan="3" id="Td1" align="left"><font color=red>*-Album không có ảnh sẽ tự động xóa khỏi hệ thống</font></td></tr>
        ...
        <td>
            <input id="Button2" type="button" value="Bỏ qua" class="btn" style="width:120px;height:25px" onClick="close_page('<%=albumid %>')"/>
        </td>
        </tr>
        </table>
        </FooterTemplate>
